...
const getTime = (time) => {
    ...
    seconds = Math.min(seconds, 100 * 60 * 60 - 1)   // max time is [99:59:59] or 1 second short of 60 hours

    // hours
    time = '00'.concat(parseInt(seconds / 3600)).slice(-2);
...
const getTimeFromNum = (time) => {
    // hours
    let result = '00'.concat(parseInt(time / 3600)).slice(-2);
